...
class UpdateProfile : Task {
    ...
    override fun run(bot: Bot, ctx: Context, settings: Settings) {
        ...
        try {
            ...
            Log.normal("XP gain: ${player.stats.experience - ctx.startXp.get()} XP; Pokemon caught/transferred/evolved: ${ctx.pokemonStats.first.get()}/${ctx.pokemonStats.second.get()}/${ctx.pokemonStats.third.get()}; Items caught/dropped: ${ctx.itemStats.first.get()}/${ctx.itemStats.second.get()}; Pokebank ${ctx.api.inventories.pokebank.pokemons.size}/${ctx.profile.pokemonStorage}; Stardust ${ctx.profile.currencies[PlayerProfile.Currency.STARDUST]}")
        } catch (e: Exception) {}
    }
}
