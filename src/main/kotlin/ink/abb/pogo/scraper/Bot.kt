...
class Bot(val api: PokemonGo, val settings: Settings, val propFile: String) {
    ...
    fun stop() {
        ...
        thread {
            ...
            settings.writeProperty(propFile,"longitude", ctx.lng.toString())
            settings.writeProperty(propFile,"latitude", ctx.lat.toString())
            ctx.server.stop()
